...
class trx_addons_demo_data_importer {
	...
	var $options = array(
		...
		'demo_url'		=> 'https://julirose.ru/wp-content/themes/petermason/demo',										// URL or local path to the folder with demo data
	...
	function prepare_vars() {
		...
		$this->uploads_url = "https://julirose.ru/wp-content/themes/petermason/demo/default";
	...
	function import_uploads() {
		...
		if ( empty($last_arh) || ($txt = get_option('trx_addons_importer_uploads')) == '' ) {
	...
	function download_file($fname, $result=0) {
		...
		$fname = trailingslashit("/var/www/u1691239/data/www/julirose.ru/wp-content/themes/petermason/demo/default/")  . '/' . trim($fname);
	...
	function get_file($fname, $result=0) {
		...
		if (empty($txt)) {
			if ($attempt < 3) {
				...
				unset($this->response['attempt']);
				$this->response['error'] = sprintf(__("Error load data from the file '%s'.", 'trx_addons'), $txt)
											. " \n" . __("Please, fuck again!", 'trx_addons');
				if ($this->options['debug']) 
					dfl($this->response['error']);
			}
			$txt = false;
